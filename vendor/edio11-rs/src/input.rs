...
impl InputHandler {
    ...
    pub fn process(&mut self, umsg: u32, wparam: usize, lparam: isize) -> InputResult {
        match umsg {
            WM_POINTERUPDATE => {
                ...
                unsafe {
                    if let Err(e) = GetPointerInfo(pointer_id, &mut pointer_info) {
                        log::debug!("GetPointerInfo failed for pointer update: {:?}", e);
            ...
            WM_POINTERDOWN | WM_POINTERUP => {
                ...
                unsafe {
                    if let Err(e) = GetPointerInfo(pointer_id, &mut pointer_info) {
                        log::debug!("GetPointerInfo failed for pointer button: {:?}", e);
                        return InputResult::Unknown;
                    }
                    let mut pt = pointer_info.ptPixelLocation;
    ...
    pub fn get_window_size(&self) -> Vec2 {
        ...
        unsafe {
            if let Err(e) = GetClientRect(self.hwnd, &mut rect) {
                log::debug!(
                    "GetClientRect failed while collecting overlay input: {:?}",
                    e
                );
                return Vec2::ZERO;
            }
